import{aH as m,al as q,l as I,ak as P,a9 as Y,aG as W,aj as K,a as Z,D as B,d as J,aE as z,g as Q,f as $,ac as ee,ad as te,k as X,a8 as ne,aD as D,am as oe,Y as O,aa as ie,I as ae,az as se,a7 as re,Z as ce,ah as le}from"./index.astro_astro_type_script_index_0_lang.DhuKjILD.js";import{G as de}from"./GLTFLoader.zTD1XmA-.js";import"./hoisted.Bjr5D1P9.js";import"./party-egg.CMa5clQ5.js";m.registerPlugin(q);{const e=document.getElementById("three-canvas"),t=e.dataset.model,n=new I(e.dataset.primary),r=new P,o=new Y(60,window.innerWidth/window.innerHeight,.1,500);o.position.z=8;const c=new W({canvas:e,alpha:!0,antialias:!0});c.setSize(window.innerWidth,window.innerHeight),c.setPixelRatio(Math.min(window.devicePixelRatio,2)),c.outputColorSpace=K,r.add(new Z(16777215,1));const v=new B(16766720,2);v.position.set(5,8,6),r.add(v);const f=new B(n,.6);f.position.set(-5,-3,-4),r.add(f);let i=null;new de().load(t,a=>{i=a.scene;const l=new J().setFromObject(i),A=l.getCenter(new z);i.position.sub(A);const y=Math.max(...l.getSize(new z).toArray());i.scale.setScalar(5/y),r.add(i)});const w=1800,u=new Float32Array(w*3);for(let a=0;a<w;a++){const l=4+Math.random()*8,A=Math.random()*Math.PI*2,y=Math.acos(2*Math.random()-1);u[a*3]=l*Math.sin(y)*Math.cos(A),u[a*3+1]=l*Math.sin(y)*Math.sin(A),u[a*3+2]=l*Math.cos(y)}const T=new Q;T.setAttribute("position",new $(u,3));const d=new ee(T,new te({size:.035,color:n,transparent:!0,opacity:.4}));r.add(d);let M=0,k=0;window.addEventListener("mousemove",a=>{M=(a.clientX/window.innerWidth-.5)*1.5,k=-(a.clientY/window.innerHeight-.5)*1.5});const _=new X;(function a(){requestAnimationFrame(a);const l=_.getElapsedTime();i&&(i.rotation.y=l*.25,i.rotation.z=Math.sin(l*.4)*.08),d.rotation.y=l*.04,o.position.x+=(M-o.position.x)*.05,o.position.y+=(k-o.position.y)*.05,o.lookAt(0,0,0),c.render(r,o)})(),window.addEventListener("resize",()=>{o.aspect=window.innerWidth/window.innerHeight,o.updateProjectionMatrix(),c.setSize(window.innerWidth,window.innerHeight),c.setPixelRatio(Math.min(window.devicePixelRatio,2))})}{let e=function(d,M){v[i].set(d,M),f[i]=w.uniforms.uTime.value,i=(i+1)%c};const t=document.getElementById("water-canvas");document.body.insertBefore(t,document.getElementById("three-canvas"));const n=new P,r=new ne(-1,1,1,-1,0,1),o=new W({canvas:t,alpha:!0});o.setSize(window.innerWidth,window.innerHeight),o.setPixelRatio(Math.min(window.devicePixelRatio,1.5));const c=12,v=Array.from({length:c},()=>new D(-2,-2)),f=new Float32Array(c).fill(-10);let i=0;const w=new oe({transparent:!0,uniforms:{uTime:{value:0},uRipples:{value:v},uRippleTimes:{value:f},uAspect:{value:window.innerWidth/window.innerHeight}},vertexShader:`
      varying vec2 vUv;
